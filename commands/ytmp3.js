...
.play https://open.spotify.com/track/2ksyzVfU0WJoBpu8otr4pz`);
            ...
            if (query.includes("spotify.com/track/")) {
                ...
                // Try Nekolabs API if PrinceTech fails
                if (!audioData) {
                    try {
                        const api = `https://archive-ui.tanakadomp.biz.id/download/spotify?url=${encodeURIComponent(query)}&apikey=`;
                        const { data } = await axios.get(api, { timeout: 20000 });
            ...
            if (!audioData) {
                ...
                // If PrinceTech search fails, try Tanaka domp API with search term
                if (!audioData) {
                    try {
                        ...
                        if (searchData?.results?.length) {
                            ...
                            const api = `https://archive-ui.tanakadomp.biz.id/download/spotify?url=${encodeURIComponent(spotifyUrl)}&apikey=`;
                            const { data } = await axios.get(api, { timeout: 20000 });
